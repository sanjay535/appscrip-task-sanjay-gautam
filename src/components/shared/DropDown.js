...
function DropDown({ values }) {
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`z-10 p-4 w-[300px] bg-slate-50 shadow-md transition-all absolute right-3 ${isOpen ? ' opacity-1 top-10' : ' opacity-0 top-[-200px]'}`}
      >
        {values.map((value) => (
          <div
            onClick={() => {
          ...
          >
            ...
            <div className={`uppercase ${value.id === choosenVal.id ? 'font-bold' : ''}`}>
              {value.text}
            </div>
          </div>
        ))}
