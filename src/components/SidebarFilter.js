...
const SidebarFilter = ({isFilterOpen}) => {
    return (
        <div className={`bg-white border border-gray-200 rounded-lg shadow z-10 px-2 lg:max-w-[300px] absolute transition-all lg:relative ${isFilterOpen? 'block md:left-0 left-7':'left-[-350px] hidden'}`}>
            ...
            {SIDEBAR_FILTER_ITEMS.map(sidebarItem=><SideBarItem key={sidebarItem.id} title={sidebarItem.title} categories={sidebarItem.categories}/>)}
        </div>
    )
}
